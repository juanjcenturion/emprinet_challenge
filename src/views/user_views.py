...
class RegisterAPIView(MethodView):
    def post(self):
        ...
        if 'error' in response:
            logger.error(f"❌ Error al registrar el usuario {username}: {response['error']}")
...
class LoginAPIView(MethodView):
    def post(self):
        ...
        if 'error' in response:
            logger.error(f"❌ Error al intentar iniciar sesión para el usuario {username}: {response['error']}")
